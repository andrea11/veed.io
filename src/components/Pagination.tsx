...
export default function Pagination({total, limitPerPage, index}: PaginationProps) {
    return (
        <div className="flex items-center justify-between border-t border-gray-200 bg-white px-4 py-3 sm:px-6">
            <div className="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
                <div>
                    <p className="text-sm text-gray-700">
                        Showing <span className="font-medium">{1 + (limitPerPage * index)}</span> to <span
                        className="font-medium">{(limitPerPage * (index + 1))}</span> of{' '}
                        <span className="font-medium">{Math.floor(total / limitPerPage)}</span> results
                    </p>
                </div>
                <div>
                    <nav className="isolate inline-flex -space-x-px rounded-md shadow-sm" aria-label="Pagination">
                        ...
                        {
                            generatePaginationLinks(total, limitPerPage, index)
                        }
                        {index === Math.floor(total / limitPerPage) ? null :
                            <Link
                                href={`/repositories/${index + 1}`}
